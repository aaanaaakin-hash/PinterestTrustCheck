...
Текущая версия: **v1.8.0** (номер виден на странице и по команде `--version`).
...
В конце разбора — блок «Почитай сам»: готовые ссылки поиска отзывов про твой домен (программа ничего не ищет, только подставляет домен в шаблон).
