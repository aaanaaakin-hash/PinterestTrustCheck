...
Текущая версия: **v1.8.1** (номер виден на странице и по команде `--version`).
...
## Принцип проверки одной фразой

Начинаем со 100 и снимаем баллы за каждый плохой сигнал: лишние перебросы,
битый сертификат, медленный ответ, молодой возраст, маскировка от бота,
чёрные списки, грязное прошлое. Молодым доменам и бесплатному хостингу режем
потолок — выше жёлтого им не подняться, даже если всё остальное чисто.
Вес каждого пункта виден в самом разборе: рядом с каждой строкой стоят баллы
«получено из возможных», а под итогом — предупреждения, за что сняли.

## Блок «Почитай сам»

Автопроверка видит только технические сигналы, а репутацию — отзывы, жалобы,
баны — видит только человек глазами. Поэтому в конце разбора программа даёт две
готовые ссылки в Google с подставленным твоим доменом (сама она ничего не ищет,
только подставляет домен в шаблон). Первая спрашивает по-русски, как Pinterest
воспринимает домен, — находит русские отзывы и жалобы. Вторая — то же самое
по-английски — находит английские источники: их обычно больше, и встроенный
ответ Гугла там полнее. Жми обе.
...
В конце разбора — блок «Почитай сам»: две ссылки в Google про твой домен (программа ничего не ищет, только подставляет домен в шаблон). Первая спрашивает по-русски «как Pinterest воспринимает домен» — ищет русские отзывы и жалобы. Вторая — то же по-английски — ищет английские источники, их обычно больше, и встроенный ответ Гугла там полнее. Жми обе.
...
**Разница честно:** без ключей проверяются все бесплатные сигналы — редиректы,
шифрование, скорость, возраст, бот, 4 списка спама, архив, сертификаты, хостинг.
Этого хватает, чтобы отсеять явно грязные домены. С ключами добавляются три
страховки: Google роняет в красный взломанные и опасные ссылки, OTX показывает
жалобы, VirusTotal — вердикты антивирусов. Проверяешь свой основной домен —
потрать 15 минут на ключи (все бесплатные). Гоняешь пачку чужих — хватит и без них.
