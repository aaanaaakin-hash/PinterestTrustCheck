...
Текущая версия: **v1.5.0** (номер виден на странице и по команде `--version`).
...
## Ещё два ключа (тоже бесплатные, тоже не обязательны)

- **OTX (жалобы на домен):** регистрация на `otx.alienvault.com` → настройки → API Integration.
  Скопируй ключ в файл `otx.key` рядом с кнопками (образец создастся сам).
- **VirusTotal (вердикты движков):** регистрация на `virustotal.com` → значок профиля → API key.
  Скопируй ключ в файл `vt.key` рядом с кнопками.

Без них программа честно пишет «не проверено», а не делает вид, что всё чисто.
