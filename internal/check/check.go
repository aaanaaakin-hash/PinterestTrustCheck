...
func VerdictOf(score int) (string, string) {
	switch {
	case score >= 80:
		return "Пинтерест будет работать хорошо — к домену вопросов нет", "ЗЕЛЁНЫЙ"
	case score >= 60:
		return "Пинтерест может резать показы — следи за первыми пинами", "ЖЁЛТЫЙ"
	case score >= 40:
		return "Пинтерест скорее всего урежет показы — готовь запасной домен", "ОРАНЖЕВЫЙ"
	default:
		return "Пинтерест почти наверняка заблокирует ссылки с этого домена", "КРАСНЫЙ"
	}
}
